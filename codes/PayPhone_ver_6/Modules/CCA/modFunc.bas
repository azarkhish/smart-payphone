...
'  PhoneStationID = DecodeLong(FileBlockTemp.FileHeaderBlock.HeaderPhoneStationSerial)
   ...
   tmpPayphoneXD = GetPayphoneXDfromDataBase(PhoneStationID)
    ...
    With de.rscmdPhoneCall
        ...
        For IndexCounter = 0 To 199
            ...
            !CallYear = CInt(Year(Now))  'CalculateYear(FileBlockTemp.FileHeaderBlock.HeaderYear) '
            !CallMonth = CByte(Month(Now)) 'DecodeBCD(FileBlockTemp.FileCallBlock(IndexCounter).CallMonth, 0)
            !CallDay = CByte(Day(Now)) 'DecodeBCD(FileBlockTemp.FileCallBlock(IndexCounter).CallDay, 0)
    ...
    With de.rscmdOtherEvent
        ...
        For IndexCounter = 0 To 9
            ...
            !OtherEventYear = CByte(FileBlockTemp.FileHeaderBlock.HeaderYear)
...
Public Sub SaveEmergencyEvent(PhoneStationSerialNo As Long, EmergencyEventCode As Byte)
     Dim tmpPayphoneXD As Double

   On Error Resume Next
       
       tmpPayphoneXD = GetPayphoneXDfromDataBase(PhoneStationSerialNo)
    ...
    With de.rscmdOtherEvent
            ...
            !OtherEventCodeRef = EmergencyEventCode '162 'line tap
...
.Close
    End With
End Sub
Public Sub UpdateShowEmergencyEventTable(PhoneStationSerialNo As Long, EmergencyEventCode As Byte, Handler As Byte)
  Dim EventNameTxt As String
   On Error Resume Next
    ...
    With de.rscmdShowEmergencyEvents
            ...
            !PhoneStationRef = PhoneStationSerialNo
            !ShowEventCodeRef = EmergencyEventCode '162 'line tap
            !ShowEventHour = Hour(Time)
            !ShowEventMinute = Minute(Time)
            !ShowEventDay = Day(Now)
            !ShowEventMonth = Month(Now)
            !ShowEventYear = Year(Now)
            !Handler = Handler
Select Case EmergencyEventCode
Case 1:
EventNameTxt = "ÔÇÑŽ ÈÇØÑí"
Case 2:
EventNameTxt = "äÈæÏä ÓíäÇá ÔÇÑŽíä"
Case 3:
EventNameTxt = "ÎÑÇÈí ÎØ"
Case 4:
EventNameTxt = "ÈÇÒ ÈæÏä ÏÑÈ"
Case 5:
EventNameTxt = "ÎÑÇÈí æÔí"
Case 6:
EventNameTxt = "äÈæÏä SAM"
Case 7:
EventNameTxt = "äÈæÏä ÓíäÇá Antitap"
Case 8:
EventNameTxt = "ÎÑÇÈí ÓÇÚÊ ÓíÓÊã"
Case 9:
EventNameTxt = "ÓæÁ ÇÓÊÝÇÏå ÇÒ ÎØ"
Case 13:
EventNameTxt = "ÎÑÇÈí ˜ÇÑÊÎæÇä"

End Select
                  !EventDiscription = EventNameTxt
