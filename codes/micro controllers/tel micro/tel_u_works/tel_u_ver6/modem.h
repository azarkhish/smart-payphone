...
//lcall CMXtonedial
	...
	//	 clr ie.7
		...
		lcall modem_replysync
////////////////////
//		mov A,#0E2H      ;addressing Rx PSK mode register
//		mov B,#11110000b ;bit 0,1,2,3 -> rx gain 0000 = -4.7
//		lcall CMXcmdsync
...
report:
mov t1_c2,#200							   ; 
lcall  start_scheduling_guard_time		   ; guard time
;------------------------------------------
	  	lcall clear_crc					   ;
	  	mov dptr,#00					   ;
		lcall save_dptr					   ; preset
										   ;
		lcall send_preset				   ;
;------------------------------------------;
	wd_0:								   ;
		...
		mov addr_hi,#00					   ;
										   ;
		mov b,#100    ;num of attempts 	   ;
	wd_1:								   ;	send PACK #x + CRC
		mov a,#PADDR					   ;
		call read_random				   ;
		jnc wd_3         				   ;
		djnz b,wd_1						   ;
...
//setb lcd_sw
//setb card_sw
	   lcall delay31m ///////////////aali bood 
		lcall receive_command_pack		  ; 
chk_nack:								  ;
		cjne a,#NACK,chk_ack
		jmp answer_is_nack    			  ;
chk_ack:								  ;
		cjne a,#ACK,check_answer		  ; answer not detected -> check again
;-----------------------------------------;
answer_is_ack:
//setb card_sw
//clr lcd_sw							  ;
		lcall send_clear_stage			  ;
//		lcall delay1s	//lll				  ;  

		lcall send_preset				  ; answer is ack
		lcall save_dptr					  ; if not end of report -> send next pack
		mov a,dph						  ;
		cjne a,#017h,wd_3				  ;
		mov a,dpl						  ;	
		cjne a,#070h,wd_3				  ;
;-----------------------------------------;		
		jmp end_of_report				  ;	all packs sent
;-----------------------------------------;
answer_is_nack:
//clr card_sw 
//setb lcd_sw						  ;
...
//		lcall delay1s				      ;
		...
		lcall send_preset				  ; answer is nack -> resend previous pack
	 	lcall previous_dptr				  ;
	 	jmp wd_0						  ;
;-----------------------------------------;	
										  ;
end_of_report:							  ;
		mov cmd_buf,#EOR				  ;
		lcall send_command_pack			  ;	 send EOR 
wait_for_eor_ack:						  ;
//	   lcall delay31m ///////////////
		lcall receive_command_pack		  ; 
		cjne a,#EOR_ACK,end_of_report	  ;	 recieve EOR_ACK
		lcall send_clear_stage			  ;	 send CLR_STG
	//	lcall delay1s					  ;  
;------------------------------------------
report_normal_end:						  ;
	clr R_error							  ;
...
lcall  start_scheduling_guard_time
	...
	SETTINGS_zx_0:                  
		...
		mov a,dpl
		cjne a,#02eh,SETTINGS_zx_0		;window size = 46	 ;#2eh 
		mov a,dph
...
call_vx_0:						 ;
	mov cmd_buf,#HI					 ; send HI
	lcall send_command_pack		 ;
call_vx_1:	
	 lcall delay31m ///////////////					 ;
	lcall receive_command_pack	 ; wait for HI_ACK
	cjne a,#HI_ACK,call_vx_0	 ; 

call_vx_2:						 ;
	mov cmd_buf,#SOR			 ; send SOR 
	lcall send_command_pack		 ;

call_vx_3:
	 lcall delay31m ///////////////
  	lcall receive_command_pack	 ; wait for GET_PACK
  	 cjne a,#GET_PACK,call_vx_3	 ;
//
call_vx_4:						 ;
	lcall send_clear_stage		 ;	send CLR_STG
								 ;

	lcall delay1s				 ; delay 1 sec	 ////////////////////////////
...
send_next_byte_clr_stg:	
lcall send_preset			    
  		mov send_buf,#0AAh				   ;
		lcall send_byte_i				   ;
 
 djnz r1,send_next_byte_clr_stg
 pop 01
ret
;-------------------------------------
check_dial_tone:
 ...
 //  lcall delay110m	  /////////////
    ...
    mov A,#0E0H         ;addressing SETUP register
    mov B,#01010011b    ;0 & Relaybit & Detector oFF & No Loop Back & 11.0592 Mhz Xtal
    lcall CMXcmdsync    ;

     mov A,#0E8H         ;addressing Rx PSK mode register
    mov B,#01111110b    ;0 & Call Progress BW & DeScramb Unlock & Rx DeScramb Y/N & EQ1 EQ0 & Rx Enable & Channel H/L
    lcall CMXcmdsync    ;
...
mov r1,#20
check_dial_tone_again:
		mov A,#0ECH
		lcall modem_replysync
		jnb B.5,no_dial_tone
   
   lcall delay110m
djnz r1,check_dial_tone_again

dial_tone_detected:
 	 setb dial_tone
jmp end_dial_tone_detection
no_dial_tone:
     clr dial_tone
end_dial_tone_detection:
